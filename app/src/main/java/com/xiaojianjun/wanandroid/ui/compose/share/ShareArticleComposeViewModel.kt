...
sealed interface ShareArticleEvent {
    data class Toast(val resId: Int) : ShareArticleEvent
}

class ShareArticleComposeViewModel : ComposeBaseViewModel() {
    ...
    fun shareArticle(title: String, link: String) {
        launchFlow(
            block = {
                ...
                _shareEvents.emit(ShareArticleEvent.Toast(R.string.share_article_success))
            },
            error = {
                _uiState.update { it.copy(submitting = false) }
            },
